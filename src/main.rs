...
fn main() {
    ...
    for k in ks {
        let mut align_times = vec![];
        let mut chain_times = vec![];
        for l in 0..num_iters {
            let n = 2_usize.pow(k as u32) as usize;
            let k = k as usize;
            let d = 6;
            let v = k - d + 1;
            let t = (k - v + 1) / 2 + 1;
            ...
            chain_times.push(now.elapsed().as_secs_f32());
            ...
            align_times.push(now.elapsed().as_secs_f32());
        }
        let align_mean = align_times.into_iter().sum::<f32>();
        let chain_mean = chain_times.into_iter().sum::<f32>();
